...
def result_plots(plot_opts: dict,
    ...
    if plot_opts["raceline"]:
        ...
        plt.figure()
        plt.plot(refline[:, 0], refline[:, 1], "k--", linewidth=0.7, label="reference line")
        ...
        plt.plot(bound1_interp[:, 0], bound1_interp[:, 1], "k-", linewidth=0.7, label="track boundaries")
        plt.plot(bound2_interp[:, 0], bound2_interp[:, 1], "k-", linewidth=0.7)
        plt.plot(trajectory[:, 1], trajectory[:, 2], "r-", linewidth=0.7, label="optimized trajectory")
        plt.plot(bound2_interp[:, 0], bound2_interp[:, 1], "k-", linewidth=0.7)
        ...
        if plot_opts["imported_bounds"] and bound1_imp is not None and bound2_imp is not None:
            plt.plot(bound1_imp[:, 0], bound1_imp[:, 1], "y-", linewidth=0.7, label="imported boundaries")
